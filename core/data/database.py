...
DB_NAME = os.path.join(APP_DIR, "math.db")
...
def create_database_and_tables():
    ...
    sql_files = [
        ...
        os.path.join(SQL_DIR, "spec_math.sql"),
    ]
...
def insert_csv_to_sor(df):
    ...
    target_col = "math_score"  # note que agora os nomes estão normalizados
...
def run_etl_sor_to_sot():
    ...
    # df_sot = df.drop(columns=['code', 'product_name', 'collagen_meat_protein_ratio_100g', 'cocoa_100g', 'chlorophyl_100g', 'carbon_footprint_100g','glycemic_index_100g', 'water_hardness_100g'], errors='ignore')
    df.to_sql("sot_food", conn, if_exists="replace", index=False)
    conn.close()
...
def run_etl_sot_to_spec_train():
    ...
    df.to_sql("spec_math", conn, if_exists="replace", index=False)
    conn.close()
...
def run_etl_for_test_data(df_test):
    ...
    df_spec = df_test[['math_score'] + [col for col in df_test.columns if col not in ['math_score']]]

    df_spec.to_sql("spec_math", conn, if_exists="replace", index=False)
    conn.close()
